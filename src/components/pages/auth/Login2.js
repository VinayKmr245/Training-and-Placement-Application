...
const UserLogin = () =>{
   ...
   const CheckSecretKey = (e)=>{
    setSecretKey(e.target.value); console.log(SecretKey);
   }
...
const [UserType,setUserType] = useState('');
 async function submitLogin (e){
      ...
      var CheckMessage="";
    await  axios.post(`http://localhost:5000/auth/${UserType}/Login`,  UserType==="Admin"? 
       { UserName:UserName,
        Password:Password  ,
        SecretKey:SecretKey
      }
...
console.log(AccessToken);
    ...
    return(
        ...
        <Box autoComplete = "off"
         sx={{display:'flex',flexFlow:'column nowrap',alignItems:'center',justifyContent:'space-around',width:'100%',position:'relative',padding:'30px 30px',maxWidth:'360px',boxShadow:4 ,borderRadius:'10px',backgroundColor:'white'}}>
            <span className="On-Box-Text">Login</span>
        <TextField
        name='password'
        autoComplete='off'
        id="standard-basic" label="UserName" variant="outlined"   onChange={(e)=>{CheckUserName(e)}} 
        ...
        <div id="CheckAdmin" className={"Check-Admin" +(UserType==="User"? " Visiblex" : " Hiddenx")}> 

        <TextField 
        name='password'
        autoComplete='off'
        id="standard-basic" label="Admin Secret Key" variant="outlined"
        inputProps={{style: {fontSize: 18,wordSpacing: 17,lineHeight: 1.5}}} InputLabelProps={{style: {fontSize: 18,wordSpacing: 17,lineHeight: 1.5}}}
        onChange={(e)=>{CheckSecretKey(e)}}             sx={{width:'100%',boxShadow:4,marginBottom:'3vh'}} />
        ...
        inputProps={{style: {fontSize: 18,wordSpacing: 17,lineHeight: 1.5}}} InputLabelProps={{style: {fontSize: 18,wordSpacing: 17,lineHeight: 1.5}}}
        onChange={(e)=>{CheckPassword(e)}}             sx={{width:'100%',boxShadow:4}} />
        <div className="Button-Holder-Log">
        <div className="SignUp-Button"  onClick={e=>{submitLogin(e)}} >Login</div>
